...
2. 장 선택은 범위(start–end)가 아니라 **칩 다중 선택** — "창세기 1장, 2장"처럼
   원하는 장만, 비연속 허용. **기본 선택은 1장 하나** — 창세기 1장만 해도
   31절이라 여러 장을 기본값으로 밀어넣으면 첫 플랜부터 과하다.
...
- 기본 선택: 그 권에서 아직 안 깬 **첫 장 하나만**. 더 하고 싶으면 사용자가
  칩을 추가로 탭한다(하단 "하루 약 N절" 미리보기가 즉시 반응하므로 늘렸을 때의
  부담이 바로 보인다).
...
- 플랜이 성경 책(ot/nt) + 섹션 플랜이었고 그 권에 안 깬 장이 남아 있으면
  "이어서: 창세기 3장" 원탭 버튼 추가 — **다음 안 깬 장 하나**만 대상으로,
  기존 플랜을 completed 처리하고 같은 마감 길이(직전 플랜의 생성~마감 일수)로
  즉시 새 플랜 생성. 여러 장을 원하면 생성 화면에서 직접 고르면 된다.
